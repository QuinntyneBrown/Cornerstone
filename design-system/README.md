...
CSS consumers import `@cornerstone/design-system/tokens.css`. Sass consumers use
`@use '@cornerstone/design-system/tokens.scss'`. Both expose the `--cs-*` properties
...
`@cornerstone/ui/styles/tokens.scss` and theme imports without a second installation.
