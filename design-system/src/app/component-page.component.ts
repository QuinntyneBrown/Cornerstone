...
export class ComponentPageComponent {
  ...
  private readonly routeState = toSignal(
    this.route.paramMap.pipe(map((params) => ({ slug: params.get('slug'), section: params.get('section') ?? 'overview' }))),
  ...
  protected memberGroups(component: ApiEntry): readonly { role: string; label: string; members: NonNullable<ApiEntry['members']> }[] {
    const groups = [
    ...
    return groups
      .map((group) => ({ ...group, members: (component.members ?? []).filter((member) => group.roles.includes(member.role)) }))
  ...
  protected signature(member: NonNullable<ApiEntry['members']>[number]): string {
    ...
    const parameters = (member.parameters ?? []).map((parameter) => `${parameter.name}${parameter.optional ? '?' : ''}: ${parameter.type}`).join(', ');
