...
export class LivePlaygroundComponent {
  ...
  protected readonly source = signal<{ readonly ts: string; readonly html: string; readonly scss: string } | undefined>(undefined);
  ...
  protected inputMembers(): NonNullable<ApiEntry['members']> {
    return (this.entry().members ?? []).filter((member) => member.role === 'input' || member.role === 'model');
  }
  ...
  protected controlKind(member: NonNullable<ApiEntry['members']>[number]): 'boolean' | 'number' | 'select' | 'json' | 'text' {
    if (member.type === 'boolean') return 'boolean';
    if (member.type === 'number') return 'number';
    if (this.options(member).length) return 'select';
    if (member.structured || /\[\]|readonly |Record<|\{|View|Data|Config|State|Item|Entry|Option/.test(member.type)) return 'json';
    return 'text';
  }
  ...
  protected currentSource(): string {
    ...
    return source?.[this.sourceLanguage()] || `No ${this.sourceLanguage().toUpperCase()} source file is required by this component.`;
  }
  ...
  protected async openSource(): Promise<void> {
    ...
    if (response.ok) this.source.set((await response.json()) as { ts: string; html: string; scss: string });
  }
  ...
  private async render(): Promise<void> {
    ...
    try {
      ...
      this.componentRef = host.viewContainer.createComponent(type, { projectableNodes: [[projected]] });
      ...
      this.renderError.set(error instanceof Error ? error.message : 'The preview could not be created.');
    }
  }
  ...
  private subscribeToOutputs(): void {
    ...
    if (!instance) return;
    for (const member of (this.entry().members ?? []).filter((item) => item.role === 'output' || item.role === 'model')) {
      const output = instance[member.name] as { subscribe?: (listener: (value: unknown) => void) => unknown } | undefined;
      output?.subscribe?.((value) => {
        const rendered = typeof value === 'string' ? value : JSON.stringify(value);
        this.events.update((events) => [`${member.name}: ${rendered ?? String(value)}`, ...events].slice(0, 12));
      });
    }
  }

  private initialValues(entry: ApiEntry): Record<string, unknown> {
    return Object.fromEntries(this.inputMembersFor(entry).map((member) => [member.name, this.initialValue(member)]));
  }

  private inputMembersFor(entry: ApiEntry): NonNullable<ApiEntry['members']> {
    return (entry.members ?? []).filter((member) => member.role === 'input' || member.role === 'model');
  }
  ...
  private initialValue(member: NonNullable<ApiEntry['members']>[number]): unknown {
    ...
    if (/Option/.test(member.type)) return [{ label: 'First option', value: 'first' }, { label: 'Second option', value: 'second' }];
