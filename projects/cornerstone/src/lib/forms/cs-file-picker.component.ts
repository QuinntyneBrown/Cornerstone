...
import { CsFileRejection } from './cs-file-rejection.interface';

@Component({
  changeDetection: ChangeDetectionStrategy.OnPush,
  selector: 'cs-file-picker',
  templateUrl: './cs-file-picker.component.html',
  styleUrl: './cs-file-picker.component.scss',
  host: { class: 'cs-file-picker' },
})
export class CsFilePickerComponent {
  readonly multiple = input(false);
  readonly accept = input('');
  readonly maxBytes = input(Number.MAX_SAFE_INTEGER);
  readonly filesSelected = output<readonly File[]>();
  readonly rejected = output<readonly CsFileRejection[]>();
