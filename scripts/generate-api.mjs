...
const libraryRoot = resolve(root, 'projects/cornerstone/src/lib');
const symbols = [];
...
function categoryFor(file) {
  return relative(libraryRoot, file).replaceAll('\\', '/').split('/')[0];
}
...
function componentMetadata(node, sourceFile) {
  for (const modifier of node.modifiers ?? []) {
    ...
    if (!ts.isIdentifier(decorator.expression) || decorator.expression.text !== 'Component') continue;
...
function classMembers(node, sourceFile) {
  ...
  for (const member of node.members) {
    if (
      ...
      if (member.modifiers?.some((modifier) => modifier.kind === ts.SyntaxKind.ProtectedKeyword)) continue;
      ...
      if (
        ...
        const role = initialized.call === 'input.required' ? 'input' : initialized.call.replace('.required', '');
  ...
  for (const statement of sourceFile.statements) {
        ...
        if (statement.exportClause && ts.isNamedExports(statement.exportClause)) {
          for (const element of statement.exportClause.elements) {
            ...
            if (localName !== element.name.text) {
              symbols.push({ kind: 'alias', name: element.name.text, sourcePath, description: `${humanize(element.name.text)} alias.` });
            }
          }
        }
      }
    ...
    if (
      ...
      const entry = {
        ...
        sourcePath,
        category: sourcePath.includes('/lib/') ? categoryFor(file) : 'public',
        description: documentation(statement, sourceFile),
      };
      if (ts.isTypeAliasDeclaration(statement)) entry.declaration = statement.type.getText(sourceFile);
...
const components = unique.filter((entry) => entry.component && !entry.sourcePath.includes('/testing/'));
for (const component of components) {
  const directory = dirname(component.sourcePath);
  const referencedTypes = new Set(
    (component.members ?? [])
      .flatMap((member) => [member.type, ...(member.parameters ?? []).map((parameter) => parameter.type)])
...
const registry = `/* Generated by scripts/generate-api.mjs. Do not edit. */\nimport { Type } from '@angular/core';\n\nexport const componentRegistry: Readonly<Record<string, () => Promise<Type<unknown>>>> = {\n${components
  .map(
    (component) =>
      `  ${JSON.stringify(component.slug)}: () => import('@cornerstone/ui').then((library) => library.${component.name} as Type<unknown>),`,
  )
  .join('\n')}\n};\n`;
const targets = [
  [resolve(root, 'docs/api-surface.md'), markdown],
  [resolve(root, 'design-system/src/generated/catalog.json'), `${JSON.stringify(catalog, null, 2)}\n`],
  [resolve(root, 'design-system/src/generated/component-registry.constant.ts'), registry],
  [resolve(root, 'design-system/public/generated/api.json'), json],
  ...components.map((component) => [
    resolve(root, `design-system/public/generated/components/${component.slug}.json`),
    `${JSON.stringify(
      {
        ...component,
  ...
  ...componentSources.map(([slug, source]) => [
    resolve(root, `design-system/public/generated/sources/${slug}.json`),
...
else console.log(`${unique.length} public symbols and ${components.length} components ${check ? 'verified' : 'documented'}.`);
